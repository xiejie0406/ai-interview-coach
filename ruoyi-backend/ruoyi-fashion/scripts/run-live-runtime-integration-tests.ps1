...
$runtimeRoot = Join-Path $repositoryRoot 'python/fashion-ai-runtime'
...
$stdinPath = Join-Path $temporaryRoot 'managed-secret.stdin.json'
$payload = @{ active = @{ keyId = 'live-runtime-test'; keyBase64 = $keyBase64 }; previous = $null } | ConvertTo-Json -Compress -Depth 3
[IO.File]::WriteAllText($stdinPath, $payload + "`n", [Text.UTF8Encoding]::new($false))
$runtimeHandle = $null

$environmentNames = @(
    'FASHION_AI_MANAGED_SECRET_STDIN',
...
try {
    [Environment]::SetEnvironmentVariable('FASHION_AI_MANAGED_SECRET_STDIN', '1', 'Process')
