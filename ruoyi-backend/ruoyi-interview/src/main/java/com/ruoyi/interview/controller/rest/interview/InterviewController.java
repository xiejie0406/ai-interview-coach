...
public class InterviewController {
    ...
    public InterviewController(CreateInterviewPlan createPlan, ConfirmInterviewPlan confirmPlan,
                               ...
                               RequestContextFactory contexts, TransactionPort transaction) {
    ...
    public ResponseEntity<SessionResponse> start(@PathVariable UUID sessionId,
                                                 ...
                                                 @Pattern(regexp = "^\"v[0-9]+\"$") String ifMatch,
                                                 HttpServletRequest request) {
        var id = ResourceId.of(sessionId);
        var expectedVersion = HttpVersionPreconditions.requireIfMatch(ifMatch);
        var context = contexts.operation(request);
        var result = transaction.required(() -> {
            var current = recover.handle(new RecoverInterview.Query(id, contexts.query(request)));
            if (current.state() == SessionState.IN_PROGRESS && current.turns().isEmpty()) {
                // 旧版本可能已经提交开始状态，却没有成功生成首题；沿用原入口恢复。
                return progress.handle(new ProgressInterview.Command(id, expectedVersion, context));
            }
            var started = start.handle(new StartInterview.Command(id, expectedVersion, context));
            // 开始状态与首题一起提交，首题失败时不留下无法继续的空会话。
            return progress.handle(new ProgressInterview.Command(id, started.snapshot().version(), context));
        });
        return ResponseEntity.ok().eTag(HttpVersionPreconditions.etag(result.version()))
                .body(SessionResponse.from(result));
    }
