...
public class JdbcCatalogRepository implements CatalogRepository, PublishedQuestionPort {
    ...
    /** 加密正文无法在 SQL 中做全文索引时，关键词扫描的单批和总上限。 */
    private static final int KEYWORD_SCAN_BATCH_SIZE = 200;
    private static final int KEYWORD_SCAN_MAX_ROWS = 10_000;
    ...
    @Override
    public Optional<RubricVersion> findLatestRubricForQuestionVersion(
            TenantId tenantId,
            ResourceId questionVersionId
    ) {
        return jdbc.query("""
                select * from catalog.rubric_version
                 where tenant_id = :tenantId and question_version_id = :questionVersionId
                 order by version_no desc, rubric_version_id desc
                 limit 1
                """, Map.of("tenantId", tenantId.value(), "questionVersionId", questionVersionId.value()),
                (row, rowNum) -> mapRubricVersion(row)).stream().findFirst();
    }

    @Override
    public CursorPage<AdminQuestionSummary> searchQuestions(
            TenantId tenantId,
        ...
        if (limit < 1 || limit > 100) {
            throw new IllegalArgumentException("catalog admin page limit must be between 1 and 100");
        }
        StringBuilder sql = new StringBuilder("""
                select q.* from catalog.question q
        ...
        state.ifPresent(value -> {
            sql.append(" and q.status = :state");
            parameters.addValue("state", value.name());
        });
        category.filter(value -> !value.isBlank()).ifPresent(value -> {
            sql.append(" and exists (select 1 from catalog.question_version qv"
                    + " where qv.tenant_id = q.tenant_id"
                    + " and qv.question_id = q.question_id"
                    + " and qv.category = :category"
                    + " and (qv.question_version_id = q.draft_version_id"
                    + " or qv.question_version_id = q.published_version_id))");
            parameters.addValue("category", value);
        });
        cursor.filter(value -> !value.isBlank()).ifPresent(value -> {
            sql.append(" and q.question_id > :cursorQuestionId");
            parameters.addValue("cursorQuestionId", decodeCursor(value));
        });
        sql.append(" order by q.question_id limit :limit");
        List<AdminQuestionSummary> rows = jdbc.query(sql.toString(), parameters,
                (row, rowNum) -> new AdminQuestionSummary(
                        ResourceId.of(row.getString("question_id")),
                        row.getString("stable_key"),
                        QuestionStatus.valueOf(row.getString("status")),
                        readVersionRef(row, "draft"),
                        readVersionRef(row, "published"),
                        new AggregateVersion(row.getLong("aggregate_version"))));
        boolean hasMore = rows.size() > limit;
        List<AdminQuestionSummary> items = hasMore
                ? List.copyOf(rows.subList(0, limit)) : List.copyOf(rows);
        Optional<String> next = hasMore && !items.isEmpty()
                ? Optional.of(encodeCursor(items.get(items.size() - 1).questionId().value())) : Optional.empty();
    ...
    public void appendQuestionVersion(QuestionVersion version) {
        ...
        MapSqlParameterSource parameters = new MapSqlParameterSource()
                ...
                .addValue("category", version.category())
        ...
        int inserted = jdbc.update("""
                insert into catalog.question_version (
                    tenant_id, question_version_id, question_id, version_no, content_hash, category,
                    ...
                    :tenantId, :versionId, :questionId, :versionNo, :contentHash, :category,
    ...
    public CursorPage<PublishedQuestionSummary> searchPublished(
        ...
        Optional<String> keyword = criteria.keyword()
                .map(value -> value.trim().toLowerCase(java.util.Locale.ROOT))
                .filter(value -> !value.isBlank());
        if (keyword.isEmpty()) {
            List<PublishedQuestionSummary> rows = queryPublishedBatch(tenantId, criteria,
                    criteria.cursor().map(JdbcCatalogRepository::decodeCursor), criteria.limit() + 1);
            return toPage(rows, criteria.limit());
        }

        /*
         * 正文是加密字段，不能只取固定 1000 行再内存过滤（会漏掉后续匹配项）。
         * 这里按 question_id keyset 分批扫描，直到收集 limit+1 个匹配或耗尽；总扫描上限
         * 为 KEYWORD_SCAN_MAX_ROWS，超限明确返回 504/DEADLINE_EXCEEDED，调用方可缩小关键词。
         */
        List<PublishedQuestionSummary> matches = new ArrayList<>(criteria.limit() + 1);
        Optional<String> scanCursor = criteria.cursor().map(JdbcCatalogRepository::decodeCursor);
        int scanned = 0;
        while (matches.size() < criteria.limit() + 1) {
            List<PublishedQuestionSummary> batch = queryPublishedBatch(tenantId, criteria, scanCursor,
                    KEYWORD_SCAN_BATCH_SIZE);
            if (batch.isEmpty()) {
                break;
            }
            scanned += batch.size();
            if (scanned > KEYWORD_SCAN_MAX_ROWS) {
                throw new ApplicationException(ApplicationErrorCode.DEADLINE_EXCEEDED,
                        "keyword search scan limit exceeded", true,
                        Map.of("scanLimit", Integer.toString(KEYWORD_SCAN_MAX_ROWS)));
            }
            for (PublishedQuestionSummary item : batch) {
                if (item.title().toLowerCase(java.util.Locale.ROOT).contains(keyword.get())) {
                    matches.add(item);
                    if (matches.size() >= criteria.limit() + 1) {
                        break;
                    }
                }
            }
            scanCursor = Optional.of(batch.get(batch.size() - 1).questionId().value());
            if (batch.size() < KEYWORD_SCAN_BATCH_SIZE) {
                break;
            }
        }
        return toPage(matches, criteria.limit());
    }

    private List<PublishedQuestionSummary> queryPublishedBatch(
            TenantId tenantId,
            QuestionSearchCriteria criteria,
            Optional<String> cursor,
            int limit
        ...
        cursor.filter(value -> !value.isBlank()).ifPresent(value -> {
            sql.append(" and q.question_id > :cursorQuestionId");
            parameters.addValue("cursorQuestionId", value);
        });
        ...
        if (!criteria.categories().isEmpty()) {
            sql.append(" and qv.category in (:categories)");
            parameters.addValue("categories", criteria.categories());
        }
        ...
        sql.append(" order by q.question_id limit :limit");
        return jdbc.query(sql.toString(), parameters, (row, rowNum) -> mapSummary(row));
    }

    private static CursorPage<PublishedQuestionSummary> toPage(
            List<PublishedQuestionSummary> rows,
            int limit
    ) {
        boolean hasMore = rows.size() > limit;
    ...
    private QuestionVersion mapQuestionVersion(ResultSet row) throws SQLException {
        ...
        return new QuestionVersion(versionId, tenantId, ResourceId.of(row.getString("question_id")),
                row.getInt("version_no"), row.getString("content_hash"), row.getString("category"),
                body.title(), body.stem(),
    ...
    private PublishedQuestionSnapshot mapSnapshot(ResultSet row) throws SQLException {
        ...
        return new PublishedQuestionSnapshot(ResourceId.of(row.getString("question_id")),
                ...
                body.title(), body.stem(), row.getString("category"), body.answerPoints(), body.followUpTemplates(),
                row.getString("difficulty"), json.readStringList(row.getString("target_roles")),
    ...
    private PublishedQuestionSummary mapSummary(ResultSet row) throws SQLException {
        ...
        return new PublishedQuestionSummary(ResourceId.of(row.getString("question_id")),
                new ImmutableVersionRef(versionId, row.getInt("version_no"), row.getString("content_hash")),
                body.title(), row.getString("category"), row.getString("difficulty"),
