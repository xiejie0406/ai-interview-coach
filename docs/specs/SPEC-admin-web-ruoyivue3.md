...
> 关联文档：`docs/licenses/ruoyi-vue3.md`（上游评审报告）、`docs/architecture/ruoyi-migration-plan.md`（迁移计划）
> 关联设计：`docs/architecture/technical-architecture.md` 第 8 节
...
2. 用户对 `docs/licenses/ruoyi-vue3.md` **签收**（commit hash / 占位策略 / frontend 处置 三选一拍板）；
3. `docs/architecture/ruoyi-migration-plan.md` 用户审过；
...
详见 `docs/architecture/ruoyi-migration-plan.md` 第 6 节。三选项（A 移 `frontend-legacy/` / B 移 `docs/reference/legacy-frontend/` / C 删除 + bundle）由用户拍板；执行时必须**保留历史证据**（git log / bundle / reference 链接）。
...
- 通过 Spring Modulith `interview-admin-bridge` module 暴露的端点工作；详见 `SPEC-backend-bridges.md`
...
按 `docs/licenses/ruoyi-vue3.md` 第 4 节"改造范围"逐文件采纳；引入动作必须通过 `git clone` 单次完成并落 `apps/admin-web/` 子目录，**禁止**把 RuoYi 全仓复制粘贴入本仓的主分支树。`.upstream/` 占位策略由用户拍板（A 永久参考仓 / B 临时 patch / C fork 名），详见 `docs/licenses/ruoyi-vue3.md` 第 8 节。
...
- [ ] 用户对 `docs/licenses/ruoyi-vue3.md` **签收**
- [ ] 用户对 `docs/architecture/ruoyi-migration-plan.md` **签收**
