...
> 关联设计：`docs/architecture/technical-architecture.md` 第 8 节（前端）
...
- 后台管理（见 `SPEC-admin-web-ruoyivue3.md`）
- 移动端原生 + 小程序（见 `SPEC-mobile-uniapp.md`）
- 后端任何端点（见 `SPEC-backend-bridges.md`）
- **不引入 portal 上游**（vben / SoybeanAdmin / mall-cook 等一律排除）

### 2.4 frontend/ portal 风格 feature 迁移（与 DEC-071 关联）

详见 `docs/architecture/ruoyi-migration-plan.md` 第 6 节"frontend/ 迁移清单"。portal-web 接收的迁移子集：
...
- REST：`/api/v1/*`，详见 `technical-architecture.md` § 13.2
...
| WebSocket 跨端复用 | mobile 也要同链路 | SPEC-backend-bridges.md 暴露统一 `/ws/v1/*` |
