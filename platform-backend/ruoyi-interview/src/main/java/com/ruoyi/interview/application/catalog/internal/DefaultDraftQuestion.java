...
public final class DefaultDraftQuestion implements DraftQuestion {
    ...
    public Result handle(TenantId catalogTenantId, Command command) {
        ...
        return transaction.required(() -> {
            ...
            String requestHash = ServerSideDigest.sha256("catalog.question.draft", targetTenant.value(),
                    command.stableKey(),
                    command.category(),
            ...
            ContentSourceReference source = command.contentSourceVersionId()
                    ...
                    .orElse(null);
            if (repository.findByStableKey(targetTenant, command.stableKey()).isPresent()) {
                throw new DomainException(com.ruoyi.interview.domain.platform.DomainErrorCode.VERSION_CONFLICT,
                        "stableKey is already in use");
            }
            Question question = Question.draft(idGenerator.nextResourceId(), targetTenant,
