...
import com.aiinterviewcoach.domain.platform.OutboxEvent;
import com.aiinterviewcoach.domain.platform.OutboxState;
...
public class JdbcOutboxRepository implements OutboxPort {
    ...
    public void save(OutboxEvent event) {
        MapSqlParameterSource parameters = new MapSqlParameterSource()
                ...
                .addValue("availableAt", com.aiinterviewcoach.adapters.outbound.persistence.shared.JdbcPersistenceSupport.writeInstant(event.availableAt()))
                .addValue("claimedBy", event.claimedBy().orElse(null))
                .addValue("claimExpiresAt", event.claimExpiresAt().map(com.aiinterviewcoach.adapters.outbound.persistence.shared.JdbcPersistenceSupport::writeInstant).orElse(null))
                .addValue("publishedAt", event.publishedAt().map(com.aiinterviewcoach.adapters.outbound.persistence.shared.JdbcPersistenceSupport::writeInstant).orElse(null))
    ...
    public List<OutboxEvent> findPublishable(Instant availableBefore, int limit) {
        ...
        return jdbc.query(SELECT_JOIN + """
                ...
                .addValue("availableBefore", com.aiinterviewcoach.adapters.outbound.persistence.shared.JdbcPersistenceSupport.writeInstant(availableBefore))
    ...
    public List<OutboxEvent> findExpiredClaims(Instant expiredBefore, int limit) {
        return jdbc.query(SELECT_JOIN + """
                ...
                """, new MapSqlParameterSource().addValue("expiredBefore",
                        com.aiinterviewcoach.adapters.outbound.persistence.shared.JdbcPersistenceSupport.writeInstant(expiredBefore))
                .addValue("limit", limit), mapper());
    }
