-- Voice stores metadata and encrypted opaque object references only; audio bytes stay in object storage.
-- Transcript text is append-only encrypted content. Execution remains NotRun.
comment on schema voice is 'Logical owner: voice module; physical owner: Flyway migrator current_user';
...
create table voice.audio_artifact (
    ...
    state varchar(32) not null check (state in ('CREATED','UPLOADING','UPLOADED','TRANSCRIBING','TRANSCRIBED','SYNTHESIZING','SYNTHESIZED','UPLOAD_FAILED','TRANSCRIBE_FAILED','SYNTHESIS_FAILED','DELETE_QUEUED','DELETE_PARTIAL','DELETED')),
    codec varchar(64),
...
create index audio_deletion_due_idx
    on voice.audio_artifact (expires_at, tenant_id, artifact_id)
    where state <> 'DELETED';
...
create table voice.transcript (
    ...
    check ((confirmed_version_id is null) = (confirmed_by is null)
...
create table voice.transcript_version (
    ...
    foreign key (tenant_id, transcript_id) references voice.transcript(tenant_id, transcript_id),
    foreign key (tenant_id, corrected_by) references identity.membership(tenant_id, user_id),
...
create table voice.turn_execution (
    ...
    session_id varchar(128) not null,
    turn_id varchar(128) not null,
    state varchar(32) not null check (state in ('IDLE','LISTENING','TRANSCRIBING','CONFIRMING','THINKING','SPEAKING','DEGRADED','CANCELLED')),
