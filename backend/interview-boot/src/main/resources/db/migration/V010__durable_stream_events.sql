-- Durable Interview/Evaluation SSE facts. This candidate has not been executed.
-- Outbox remains the external-delivery ledger; stream_event is an owner-scoped recovery read model.
...
comment on table platform.stream_head is
    'Atomic per-tenant stream sequence owner; retention never decrements or reuses its high-water mark';
...
comment on table platform.stream_event is
    'Append-only allowlisted SSE recovery facts; answer/transcript/prompt/model/payment/storage-key content is forbidden';

create index stream_event_replay_idx
    on platform.stream_event (tenant_id, stream_type, stream_id, sequence, event_id);

create index stream_event_retention_idx
    on platform.stream_event (expires_at, tenant_id, stream_type, stream_id, sequence);
...
begin
    ...
    if new.latest_sequence < old.latest_sequence
        ...
        raise exception 'stream sequence must advance exactly once or remain unchanged' using errcode = '55000';
    end if;
    if new.latest_sequence = old.latest_sequence
        and new.latest_event_id is distinct from old.latest_event_id then
        raise exception 'stream event id cannot change without sequence advance' using errcode = '55000';
    end if;
    if new.latest_sequence = old.latest_sequence + 1
        and (new.latest_event_id is null or new.latest_event_id is not distinct from old.latest_event_id) then
        raise exception 'stream sequence advance requires a new event id' using errcode = '55000';
    end if;
