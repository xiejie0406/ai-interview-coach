...
public final class DefaultIdempotencyGuard implements IdempotencyGuard {
    ...
    private IdempotencyRecord load(com.aiinterviewcoach.application.shared.OperationContext context,
     ...
     */
    private static com.aiinterviewcoach.domain.platform.TenantId tenantScope(
    ...
    private static Decision decision(IdempotencyRecord record, Instant now) {
        if (record.state() == com.aiinterviewcoach.domain.platform.IdempotencyState.PROCESSING) {
        ...
        if (record.state() == com.aiinterviewcoach.domain.platform.IdempotencyState.SUCCEEDED) {
            return new Decision(DecisionType.REPLAY_SUCCESS, record.resourceReferences(), record.responseStatus(),
                    java.util.Optional.empty());
        }
        if (record.state() == com.aiinterviewcoach.domain.platform.IdempotencyState.FAILED_REPLAYABLE) {
