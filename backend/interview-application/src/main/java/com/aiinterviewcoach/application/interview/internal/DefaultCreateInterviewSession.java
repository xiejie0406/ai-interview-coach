    ...
    public com.aiinterviewcoach.application.interview.InterviewSessionSnapshot handle(Command command) {
        return transaction.required(() -> {
            ...
            if (decision.type() == IdempotencyGuard.DecisionType.REPLAY_SUCCESS) {
                ...
                return repository.findSession(owner.tenantId(), com.aiinterviewcoach.domain.platform.ResourceId.of(sessionId))
            ...
            if (repository.findSessionByPlan(owner.tenantId(), plan.id(), plan.planVersionNo()).isPresent()) {
                throw new com.aiinterviewcoach.domain.platform.DomainException(
                        com.aiinterviewcoach.domain.platform.DomainErrorCode.SESSION_ALREADY_ACTIVE,
