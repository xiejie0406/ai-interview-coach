...
import com.aiinterviewcoach.application.catalog.SubmitQuestionForReview;
...
public final class DefaultSubmitQuestionForReview implements SubmitQuestionForReview {
    ...
    public DefaultSubmitQuestionForReview(CatalogRepository repository, CatalogAccess access,
    ...
    public Result handle(Command command) {
        return transaction.required(() -> {
            ...
            String requestHash = ServerSideDigest.sha256("catalog.question.review", command.questionId().value(),
            ...
            IdempotencyGuard.Decision decision = idempotency.begin(new IdempotencyGuard.BeginCommand(
                    "catalog.question.review", requestHash, command.context()));
            if (decision.type() == IdempotencyGuard.DecisionType.IN_PROGRESS) {
                throw new ApplicationException(ApplicationErrorCode.IDEMPOTENCY_IN_PROGRESS,
                        "question review request is already processing", true, Map.of());
            }
            ...
            question.submitForReview(command.expectedVersion(), command.context().eventContext());
            repository.saveQuestion(question);
            domainEvents.append(question.pullDomainEvents());
            idempotency.succeed(new IdempotencyGuard.CompleteCommand(
                    "catalog.question.review", requestHash,
