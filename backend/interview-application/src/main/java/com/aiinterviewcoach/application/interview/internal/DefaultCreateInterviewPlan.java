    ...
    public InterviewPlanView handle(Command command) {
        return transaction.required(() -> {
            ...
            if (decision.type() == IdempotencyGuard.DecisionType.REPLAY_SUCCESS) {
                ...
                return repository.findPlan(owner.tenantId(), com.aiinterviewcoach.domain.platform.ResourceId.of(planId))
            ...
            if (!resolved.expiresAt().isAfter(command.context().requestedAt())) {
                throw new com.aiinterviewcoach.domain.platform.DomainException(
                        com.aiinterviewcoach.domain.platform.DomainErrorCode.PLAN_EXPIRED,
