...
def _local_registry(schema_path: Path) -> SchemaRegistry:
    ...
    for candidate in root.rglob("*.json"):
        ...
        resource: Resource[Schema] = Resource.from_contents(
            cast(Schema, document), default_specification=DRAFT202012
        )
...
def validate_instance_file(
    schema_path: Path, instance_path: Path, fragment: str | None = None
) -> None:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="校验 Aden 本地 Agent JSON 契约")
    commands = parser.add_subparsers(dest="command", required=True)
    schema = commands.add_parser("schema")
    schema.add_argument("schema_path", type=Path)
    instance = commands.add_parser("instance")
    instance.add_argument("schema_ref")
