    ...
    *,
    ...
    async def get_health() -> HealthResponse:
        return HealthResponse(
            ...
            status="ok",
            provider=(
                ProviderCapability(mode="demo", enabled=True)
                if demo_mode
                else ProviderCapability(mode="disabled", enabled=False)
            ),
        )
        ...
        return CapabilitiesResponse(
            ...
            operations=(
                CapabilityOperation(
                    ...
                    versions=("1.0",),
                    status="available" if demo_mode else "unavailable",
                    reason="demo_mode" if demo_mode else "provider_disabled",
                ),
                CapabilityOperation(
