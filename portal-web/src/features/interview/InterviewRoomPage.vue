...
function toggleTextComposer() {
  ...
  if (!showTextComposer.value) answer.value = ''
}
  ...
  <section class="voice-workspace">
    ...
    <main class="voice-panel conversation-panel">
      <header class="conversation-header">
        <div class="topic-heading"><span class="topic-icon">◉</span><div><strong>Java 后端 · 自适应语音面试</strong><small>AI 生成题目 · AI 语音提问 · 实时转写</small></div></div>
      ...
      <div ref="messagesElement" class="conversation-scroll" aria-live="polite">
        <div class="conversation-inner">
          ...
          <div v-if="voiceState === 'LISTENING' || partialTranscript" class="live-card"><div class="live-top"><span class="listen"><i class="pulse"></i>正在实时转写</span><span>语音模式</span></div><div class="live-text">{{ partialTranscript || '直接回答即可，系统会实时识别你的语音。' }}<i class="caret"></i></div></div>
          ...
          <div v-if="snapshot?.state === 'COMPLETED'" class="completion-state"><strong>面试已完成</strong><span>本次面试已结束，报告将在处理完成后提供。</span></div>
        </div>
      </div>
      <footer class="voice-dock">
        ...
        <div v-else-if="currentTurn && !terminal" class="dock-content">
          ...
          <div v-if="voiceMode && !showTextComposer && !transcript" class="voice-surface">
            <div class="dock-status"><b>{{ aiStateLabel }}</b><span>{{ voiceState === 'LISTENING' ? `已上传 ${recordedBytes} bytes` : '直接说话即可，系统会自动识别停顿' }}</span></div>
          ...
          <p class="dock-caption">{{ voiceMode && !showTextComposer && !transcript ? '语音优先 · AI 朗读结束后会自动开始收音' : '发送后将成为正式面试回答，AI 会继续追问。Ctrl + Enter 快速发送。' }}</p>
        </div>
      </footer>
    </main>
    ...
    <aside class="voice-panel status-panel" aria-label="AI 面试状态">
      ...
      <div class="status-scroll">
        ...
        <section class="status-actions"><button v-if="snapshot?.allowedCommands?.includes('PAUSE')" class="secondary-action" type="button" :disabled="busy" @click="command('pause')">暂停面试</button><button v-if="snapshot?.allowedCommands?.includes('RESUME')" class="secondary-action" type="button" :disabled="busy" @click="command('resume')">继续面试</button><button v-if="snapshot?.allowedCommands?.includes('SKIP')" class="secondary-action" type="button" :disabled="busy" @click="command('skip')">跳过当前题</button><button v-if="snapshot?.allowedCommands?.includes('COMPLETE')" class="danger-outline" type="button" :disabled="busy" @click="command('complete')">提前结束</button></section>
      </div>
