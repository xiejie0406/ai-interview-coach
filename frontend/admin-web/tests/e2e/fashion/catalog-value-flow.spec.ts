...
test('价格和库存严格全量、显式零与恢复均只调用 Java API', async ({ page }) => {
  ...
  await page.locator('.catalog-import input[type=file]').setInputFiles(
    '../../ruoyi-backend/ruoyi-fashion/src/main/resources/fashion/import-templates/price-v1.csv')
  ...
  await page.locator('.catalog-import input[type=file]').setInputFiles(
    '../../ruoyi-backend/ruoyi-fashion/src/main/resources/fashion/import-templates/stock-v1.csv')
  await page.getByRole('button', { name: '严格校验并预览' }).click()
