...
| --- | --- | --- |
| `platform-backend/ruoyi-aden` | 保持 1 个 Maven 业务模块，由现有 `ruoyi-admin` 承载 | Workspace、Task、Runner、Event/Audit 四个当前领域闭环；Agent/Action 不落真实运行链 |
| `aden-desktop/` | 保持 1 个 npm / Electron 工程，main、preload、renderer 三个信任层 | 合成登录、任务中心、Runner 状态、能力总览、REST/SSE 恢复和安全负向行为 |
| `aden-runner/` | 新增 1 个 Python 3.12 工程 | 六个低权限包组成的 Runner simulator，无 UIA、Shell、浏览器、文件或外发能力 |
| `aden-agent-runtime/` | 新增 1 个 Python 3.12 工程 | 九个逻辑包的离线 Fake 骨架；只产候选，不接真实 Provider、不接服务端 AgentJob 链 |
...
| 治理 Stage 9 用户验收 | `NotStarted`；只能由用户 / 业务负责人形成决定，Codex 不代签 | 发生验收时创建 `验收记录.md` |
...
| 契约与源码 | `contracts/aden/`、`platform-backend/ruoyi-aden/`、`aden-desktop/`、`aden-runner/`、`aden-agent-runtime/` | 本地文件变化；只含合成 fixture | Approved | Approved | 新顶层工程、Java 子模块、真实连接器或超出 Feature |
| RuoYi 窄集成 | `platform-backend/pom.xml`、`ruoyi-admin/pom.xml`、`ruoyi-admin/src/main/resources/application*.yml`、`platform-backend/sql/aden-permissions.sql` | 仅承载 Aden；不顺手重构共享认证 / 数据源 | Approved | Approved | 改共享认证语义、其他业务模块或默认角色权限 |
...
contracts/aden/       # 不存在
aden-runner/          # 不存在
aden-agent-runtime/   # 不存在
```

工作区当前还可见 `platform-backend/ruoyi-aden/target/`、`aden-desktop/node_modules/`、`aden-desktop/out/` 和 TypeScript build info 等生成物；它们不属于目标源码树，也不构成本轮验证证据。
...
platform-backend/ruoyi-admin/
├── pom.xml                                      # 仅必要承载依赖
└── src/main/resources/application*.yml          # Aden 默认关闭及 test/local 配置
platform-backend/sql/aden-permissions.sql
...
platform-backend/{pom.xml,ruoyi-aden/pom.xml,ruoyi-admin/pom.xml}
aden-desktop/{package.json,package-lock.json}
aden-runner/{pyproject.toml,uv.lock,README.md}
aden-agent-runtime/{pyproject.toml,uv.lock,README.md}
...
```text
platform-backend/ruoyi-aden/pom.xml
platform-backend/ruoyi-aden/src/main/java/com/ruoyi/aden/configuration/
platform-backend/ruoyi-aden/src/main/java/com/ruoyi/aden/migration/
platform-backend/ruoyi-aden/src/main/java/com/ruoyi/aden/infrastructure/time/
platform-backend/ruoyi-aden/src/main/resources/db/aden-migration/V1__*.sql ... V4__*.sql
platform-backend/ruoyi-aden/src/test/java/com/ruoyi/aden/{migration,architecture}/
platform-backend/ruoyi-admin/{pom.xml,src/main/resources/application*.yml}
platform-backend/sql/aden-permissions.sql
```
...
| ADEN-TASK-013 | 本机一次性 MySQL 8 先导入只读基线 `platform-backend/sql/ry_20260417.sql`，再验证 fresh migrate、显式 baseline 0、migrate、validate、重复 migrate、checksum、空库 / 错库保护和失败恢复；测试 seed 独立于 migration，不依赖 Docker Desktop |
...
```text
platform-backend/ruoyi-aden/src/main/java/com/ruoyi/aden/
├── domain/workspace/
├── application/workspace/
├── infrastructure/security/
├── infrastructure/persistence/mapper/Aden*Workspace*Mapper.java
└── api/{operator/{WorkspaceController,admin/*},common,error}
platform-backend/ruoyi-aden/src/main/resources/mapper/aden/Aden*Workspace*Mapper.xml
platform-backend/ruoyi-aden/src/test/java/com/ruoyi/aden/{security,integration}/
```
...
platform-backend/ruoyi-aden/src/main/resources/mapper/aden/Aden{Task,TaskStep,Event,Outbox,Inbox,Audit}*Mapper.xml
platform-backend/ruoyi-aden/src/test/java/com/ruoyi/aden/{domain,integration,concurrency}/
```
...
platform-backend/ruoyi-aden/src/main/resources/mapper/aden/Aden*Runner*Mapper.xml
platform-backend/ruoyi-aden/src/test/java/com/ruoyi/aden/{security,concurrency,integration}/
aden-runner/{pyproject.toml,uv.lock,README.md,src/aden_runner/,tests/}
```
...
platform-backend/ruoyi-aden/src/test/java/com/ruoyi/aden/{contract,api,security,integration}/
```
...
└── features/
    ...
    ├── runner-status/
    └── capability-overview/
aden-desktop/tests/{component,integration,e2e}/
aden-desktop/scripts/run-synthetic-e2e.*
platform-backend/ruoyi-aden/src/test/resources/fixtures/
platform-backend/sql/ry_20260417.sql                 # 只读 RuoYi 基线输入
aden-runner/src/aden_runner/fixtures/
```
...
合成 E2E 在隔离 test profile 关闭 captcha 完成无人值守主链；captcha-enabled challenge 的格式、图片与错误路径由 main / preload 自动化覆盖。真实 captcha-enabled 人工输入属于治理 Stage 9 UAT 输入，仍须命中隔离 RuoYi `/captchaImage → /login → /getInfo → /logout` 并记录脱敏结果，Codex 不代签。禁止 OCR、固定验证码或新增生产绕过接口。测试 seed 只能由 E2E setup 写入本轮一次性 MySQL，不进入普通 migration、应用启动或示例配置。
...
platform-backend/ruoyi-aden/src/test/            # contract/integration/security/concurrency/fault
aden-runner/tests/                               # contract/fault
aden-agent-runtime/tests/                        # contract/golden/fault
aden-desktop/tests/                              # unit/component/integration/e2e
...
- 本版已于 2026-09-12 获用户整体批准且未保留例外；2026-09-13 完成 Stage 7～8，下一步进入 Stage 9 用户验收。后续新增例外只重开受影响决定和阶段。
