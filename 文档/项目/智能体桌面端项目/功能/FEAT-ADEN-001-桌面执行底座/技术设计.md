...
| --- | --- | --- |
| `ruoyi-backend/ruoyi-aden` | 已存在 1 个 Maven 模块，只有任务状态骨架 | 在这个模块内实现控制面；不再新增 Java 子模块 |
| `frontend/desktop/aden-desktop/` | 已存在 1 个 npm / Electron 工程，只有静态壳 | 仍保持一个工程，按 9 个内部逻辑模块演进 |
| `python/aden-agent-runtime/` | 不存在 | 当前新增低权限、无网络 Fake 骨架；真实 Agent Worker / Provider 另走后续 PoC |
| `python/aden-runner/` | 不存在 | 当前优先新增无权限 `runner-simulator`；真实 Runner 另走 L3 |
