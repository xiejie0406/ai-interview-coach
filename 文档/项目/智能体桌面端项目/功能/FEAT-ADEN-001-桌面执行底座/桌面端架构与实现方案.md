...
> 事实边界：本文拥有 `aden-desktop/` 内部进程、模块、状态、IPC、安全和打包设计；服务端 API 字段以 `IMP-01` 建立的 `contracts/aden/` 为准
...
桌面工程保持一个顶层项目 `aden-desktop/`，不再新增平行前端项目。代码与信任边界固定为 Electron `main → preload → renderer` 三层：
...
- 顶层桌面工程：**1 个**，继续使用现有 `aden-desktop/`。
