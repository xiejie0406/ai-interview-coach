...
> 2026-09-09 首版收敛入口：[FEAT-INTERVIEW-002](../功能/FEAT-INTERVIEW-002-商业邀测/README.md)。六项产品取舍、0.2.0 候选契约和 S1 执行包分别在其 spec / design / tasks 中维护；本文及相邻 0.1.0 契约保留为广义方案输入，不作为新首版的完整执行规格。本段只增加入口，不改变以下历史审阅和批准事实。
...
> 输入：[产品与竞品讨论稿](../调研/2026-09-07-商业语音面试双智能体方案讨论.md)、当前工作区源码与 contracts；读取日期 2026-09-07  
> Feature 当前主阶段仍以[控制页](../功能/FEAT-INTERVIEW-001-AI面试教练/README.md)为准。本文不推进主阶段，不表示阶段 3/4 门已通过，不替代既有 Approved 收敛设计。  
> 本轮仅编写文档与附属候选契约；未修改生产代码、运行系统、调用付费服务或发布价格。

> 流程审阅补充（2026-09-08）：[全流程还原与设计缺口](../调研/2026-09-08-商业语音面试全流程审阅.md)。按开场、逐题问答、HR/技术交接、结束评分、权益与复练展开，登记 13 项待闭合问题；新建议未自动覆盖本稿或附属 JSON 的 0.1.0 候选语义。
...
| 要了解什么 | 入口 |
| --- | --- |
| 为什么做、竞品证据、产品旅程与线框 | [产品讨论稿](../调研/2026-09-07-商业语音面试双智能体方案讨论.md) |
| 每层职责、文件安排、Agent 与接口解释 | 本文第 2–12 节 |
| REST 候选契约 | [openapi.json](商业面试契约/openapi.json) |
| 数据、Agent 和事件候选 Schema | [schemas.json](商业面试契约/schemas.json) |
| 语音与事件通道候选契约 | [asyncapi.json](商业面试契约/asyncapi.json) |
...
| --- | --- | --- |
| `ruoyi-backend/ruoyi-admin` | RuoYi 启动与配置容器 | 继续作为唯一 Java 服务入口 |
| `ruoyi-backend/ruoyi-interview/.../application` | interview、voice、evaluation、agent、platform 等用例和 Port | 演进现有用例，不在 Controller 内重建业务流程 |
...
| `frontend/portal-web/src/features/interview` | 配置与面试室页面，当前页面承担大量媒体与状态逻辑 | 渐进提取 API、媒体与会话编排，保留页面入口 |
...
ruoyi-backend/ruoyi-interview/src/main/java/com/ruoyi/interview/
...
ruoyi-backend/ruoyi-interview/src/main/resources/
...
文档/项目/面试项目/架构/商业面试契约/        [本轮新增] 独立 Draft 契约资产
```
...
当前可评审的结果：分层/模块/目录、角色和提示词、工具白名单、候选REST与流式契约、状态与事务、评分、数据、价格与运行边界。正式实现任务仍以Feature的`任务清单.md`为唯一入口，本稿不创建重复可执行任务源。
