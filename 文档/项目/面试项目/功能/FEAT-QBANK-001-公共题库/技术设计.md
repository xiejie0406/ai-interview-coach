...
题库只运行在 `ruoyi-backend/ruoyi-admin + ruoyi-interview`，管理端使用 `frontend/admin-web`，用户端使用 `frontend/portal-web`。不新增第二个 Spring Boot main、账号表、Session Cookie、CSRF 体系或独立权限实现。
...
- `frontend/admin-web`：内容治理工作台；不得调用公开写接口代替 Admin 工作流。
- `frontend/portal-web`：公开浏览和个人答案；不得展示公共题目创建入口。
