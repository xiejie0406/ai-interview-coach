# 窗口 05 提示词：Feature 控制页、任务清单.md 与执行包
...
任一项不满足：只读报告阻断，不创建 任务清单.md，不创建源码。

激活后完整读取：项目/用户 AGENTS.md、项目规范索引、Approved PRD/原型/技术设计、docs/phases/第01期-独立工程契约与模块边界地基.md、多窗口并行开发计划.md、就绪定义验证与验收计划.md，以及 agent-governance、development-spec、quality-delivery-spec、documentation-spec。
...
至少建立：README.md、功能规格.md（只链接/提取批准范围，不复制整个产品 PRD）、技术设计.md、任务清单.md、验证记录.md 空的计划结构。L3 跳过任何产物需写理由。
