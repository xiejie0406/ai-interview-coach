...
文件编号在正式 `任务清单.md` 按实际基线分配；此处的 `V###` 只是顺序占位，不可直接当最终文件名。
...
TanStack Query 负责服务端状态；Zustand 只负责短期交互/设备/连接；服务端仍是 session、tenant、quota、deletion 真相。每个页面计划相邻放 `.test.tsx` 和 Playwright 场景引用，具体数量由 Phase `任务清单.md` 决定。
...
本路线实施时，“计划修改”优先限于根 POM、对应 module POM、Boot 装配、契约索引、Router、共享 API/错误层和明确的跨域事件；逻辑域不得为了便利修改其他域内部文件。每一期具体新增/修改树见对应 `phase-XX.md`，正式文件清单只能在批准的 Feature `任务清单.md` 中落定。
