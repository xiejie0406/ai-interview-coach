...
| 十八期路线 | 已合并为单份 Draft 路线，可指导切片顺序 | Phase 只做编排，不替代 Feature 任务清单.md |
...
| 窗口 | 文件 | 是否写代码 | 主要目标 |
|---|---|---:|---|
| 20 | 窗口20-第3波协调与冲突控制.md | 否 | 登记目录 owner、启动门和交接，阻止并发覆盖与状态误报 |
| 21 | 窗口21-评测报告与学习.md | 是 | Evidence、Rubric Judge、Report、Learning Coach |
| 22 | 窗口22-持久化与数据库迁移.md | 是 | JDBC/SQL 候选、tenant 组合约束、outbox/job/idempotency |
| 23 | 窗口23-核心应用服务.md | 是 | identity/catalog/practice/interview/billing/governance 用例实现 |
| 31 | 窗口31-核心应用边界与幂等收口.md | 是 | 修复跨域 internal import、幂等四态、精确回放与匿名注册 scope |
| 27 | 窗口27-评测报告学习契约收口.md | 是 | 把窗口 21 骨架对齐四个 JSON Schema、REST 状态和 durable Job receipt |
| 28 | 窗口28-面试快照与语音答案原子收口.md | 是 | 对齐权威 InterviewSnapshot，并让 Transcript 确认、AnswerVersion 与 next Job 原子提交 |
| 30 | 窗口30-持久化契约回填.md | 是 | 把窗口 22 的 Repository/SQL 回填到 27/28 收口后的权威模型 |
| 32 | 窗口32-持久化SSE流与重放.md | 是 | durable stream_event、原子 sequence、Last-Event-ID replay 与 cursor expiry |
| 24 | 窗口24-REST-SSE-WebSocket适配器.md | 是 | inbound REST/SSE/WebSocket 映射与安全失败 |
| 25 | 窗口25-前端契约对齐.md | 是 | 前端 API facade、页面恢复、错误/空态和敏感缓存 |
| 29 | 窗口29-启动会话安全与组件装配.md | 是 | Session/tenant/CSRF/admin 过滤链与 use-case Bean 条件装配 |
| 26 | 窗口26-集成静态审查.md | 否 | 只读检查冲突、契约裂缝和下一轮 blockers |
...
- Feature 任务清单.md / 执行包仍需用户批准后才能成为开发事实源。
...
后续推荐改用 [`第4波入站与前端落地多窗口提示词索引.md`](../阶段/窗口提示词/第4波入站与前端落地多窗口提示词索引.md)：
