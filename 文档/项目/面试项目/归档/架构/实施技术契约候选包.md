...
> 设计版本：v0.1-candidate  
> Spec 基线：[`../product/产品需求文档.md`](../../产品/产品需求文档.md) v0.2 Draft、[`技术架构.md`](技术架构.md) v0.1 Draft、[`../phases/技术架构审查与边界深化.md`](../阶段/技术架构审查与边界深化.md) Draft  
...
- 当前主阶段仍是阶段 3 功能规格，状态 `WaitingForApproval`；阶段 4 原型、Approved PRD/设计、Feature `任务清单.md` 与执行包尚不存在。

因此本文遵守以下使用规则：

1. `DES-PACK-*` 是候选设计 ID，不是 Approved 契约；批准后应由设计 owner 回写正式设计或明确本文成为批准基线。
2. 数据表、类名、API 与事件名均为“计划创建”；实际文件只能由后续 Approved `任务清单.md` 决定。
...
- 不把本文当作单 Feature 的 `任务清单.md`，也不在 Phase 中维护第二套 TASK 正文。
...
- Flyway 从 Phase 01 进入，migration 计划位于 `interview-boot/src/main/resources/db/migration/`；`V###` 仅占位，正式编号在单一 owner 的 `任务清单.md` 分配。
...
本节的 `PACK-DEC-*` 只是本文内的待决问题，不能替代 [`../decisions/决策登记表.md`](../../决策/决策登记表.md)。正式批准时应合并到对应 `DEC-*` 或设计评审记录，避免第二份决策事实源。
...
未关闭的决策不会阻止本文继续评审，但会阻止对应 Phase 的 `任务清单.md` 或真实外部链路。实现窗口不得选择“默认值”把 Pending 变成事实。
...
4. 每个可执行 Feature 建立唯一 `docs/features/<FEAT-ID>-<slug>/任务清单.md`，包含 TASK、依赖、精确文件范围、验证、执行包和独立授权。
...
| 无 Approved Feature 资料包 | 当前只有总 PRD/路线 Draft，无各 Feature tasks/执行包 | 不能安全分配编码窗口或修改范围 | 建立 Feature 控制页、Approved Spec/design、唯一 任务清单.md |
...
- 风险：L3。
- 本轮产物：唯一文件 `docs/architecture/实施技术契约候选包.md`。
...
- 下一阶段门：关闭第 23.2 节阻断，批准产品/原型/设计，再由唯一 Feature `任务清单.md` 把 `DES-PACK-*` 转成可执行 TASK 和独占文件包。
