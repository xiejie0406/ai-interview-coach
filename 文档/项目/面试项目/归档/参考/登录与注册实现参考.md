...
> 2026-09-10 阅读提示：本文是已删除旧 `frontend/` / `backend/` 的认证实现快照；路径和 Cookie Session 方案不代表当前 RuoYi JWT 实现。
...
> 关联：[`实现参考索引`](../../参考/README.md)、[`Identity OpenAPI`](../../../../../contracts/openapi/identity.yaml)、[`技术架构`](../架构/技术架构.md)、[`FEAT-INTERVIEW-001 控制页`](../../功能/FEAT-INTERVIEW-001-AI面试教练/README.md)
...
路由当时在历史路径 `frontend/src/app/router.tsx` 中定义：
...
旧前端通过历史路径 `frontend/src/features/identity/api/identityApi.ts` 中的 `identityApi.register` 调用：
