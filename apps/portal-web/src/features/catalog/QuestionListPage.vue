...
const newQuestion = ref({ title: '', prompt: '', answer: '' })
...
async function createQuestion() {
  if (!activeCategory.value || !newQuestion.value.title.trim() || !newQuestion.value.prompt.trim() || !newQuestion.value.answer.trim()) return
  ...
  try {
    const created = await apiRequest<{ id: string }>('/questions', {
      ...
      body: jsonBody({
        category: activeCategory.value,
        difficulty: activeDifficulty.value || 'MID',
        title: newQuestion.value.title.trim(),
        prompt: newQuestion.value.prompt.trim(),
        systemAnswer: newQuestion.value.answer.trim(),
      }),
    })
    showCreate.value = false
    newQuestion.value = { title: '', prompt: '', answer: '' }
    await loadQuestions()
    selectQuestion(created.id)
  } catch (cause) {
    createError.value = cause instanceof Error ? cause.message : '题目新增失败'
  } finally {
  ...
  <section class="qbank-workbench" :class="{ 'modules-collapsed': collapsed }" aria-label="面试题库学习工作台">
    ...
    <section class="qbank-questions" aria-label="模块题目">
      <div class="qbank-column-header">
        ...
        <button v-if="activeModule && session.account" class="qbank-quiet-button" type="button" @click="showCreate = true">+ 添加</button>
      </div>
    ...
    <div v-if="showCreate" class="qbank-dialog-backdrop" role="presentation" @click.self="showCreate = false">
      <section class="qbank-dialog" role="dialog" aria-modal="true" aria-labelledby="create-question-title">
        <h2 id="create-question-title">添加公共题目</h2>
        <p>新题保存后会立即进入“{{ activeModule?.[1] }}”公共题库，所有用户均可查看。</p>
        <label>题目标题<input v-model="newQuestion.title" maxlength="500" /></label>
        <label>题目描述<textarea v-model="newQuestion.prompt" rows="5" maxlength="8000" /></label>
        <label>基础答案<textarea v-model="newQuestion.answer" rows="10" maxlength="20000" /></label>
        <p v-if="createError" class="qbank-error" role="alert">{{ createError }}</p>
        <div class="qbank-form-actions">
          <button class="qbank-quiet-button" type="button" @click="showCreate = false">取消</button>
          <button
            class="qbank-primary-button"
            type="button"
            :disabled="createBusy || !newQuestion.title.trim() || !newQuestion.prompt.trim() || !newQuestion.answer.trim()"
            @click="createQuestion"
          >{{ createBusy ? '保存中…' : '确认公开' }}</button>
        </div>
      </section>
    </div>
  </section>
