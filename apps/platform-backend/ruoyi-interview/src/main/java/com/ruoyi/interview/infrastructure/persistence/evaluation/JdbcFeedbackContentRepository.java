...
public class JdbcFeedbackContentRepository implements FeedbackContentPort {
    ...
    public String store(
        ...
        if (stored.size() != 1
                || !evaluationId.value().equals(stored.getFirst().evaluationId())
                || !userId.value().equals(stored.getFirst().userId())
                || !contentRef.equals(stored.getFirst().contentRef())
                || !comment.equals(stored.getFirst().comment())) {
