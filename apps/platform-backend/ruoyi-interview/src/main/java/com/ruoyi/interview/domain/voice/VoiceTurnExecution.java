...
public final class VoiceTurnExecution {
    ...
    public void acceptClientSequence(long sequence, AggregateVersion expectedVersion) {
        ...
        DomainPreconditions.require(state != VoiceTurnState.IDLE && state != VoiceTurnState.CANCELLED,
                DomainErrorCode.INVALID_STATE, "voice turn is not accepting protocol frames");
    ...
    public void finishThinking(AggregateVersion expectedVersion) {
        transition(VoiceTurnState.THINKING, VoiceTurnState.IDLE, expectedVersion);
    }

    public void emitServerSequence(long sequence, AggregateVersion expectedVersion) {
        expected(expectedVersion);
        DomainPreconditions.require(state != VoiceTurnState.CANCELLED && socketGeneration > 0,
