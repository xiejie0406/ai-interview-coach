...
```powershell
Copy-Item platform-backend/ruoyi-fashion/scripts/candidate-validation-manifest.example.json candidate-validation-manifest.json
pwsh -NoProfile -ExecutionPolicy Bypass -File platform-backend/ruoyi-fashion/scripts/test-candidate-validation-preflight.ps1 -ManifestPath candidate-validation-manifest.json -OutputPath candidate-preflight-report.json
```
...
```powershell
Copy-Item fashion-ai-runtime/evals/visual-trial-results.example.json visual-trial-results.json
$env:PYTHONUTF8 = '1'
[Console]::OutputEncoding = [Text.UTF8Encoding]::new($false)
uv run --project fashion-ai-runtime fashion-evaluate-visual-trial visual-trial-results.json visual-trial-report.json
```
...
```powershell
Copy-Item fashion-ai-runtime/evals/candidate-readiness-evidence.example.json candidate-readiness-evidence.json
$env:PYTHONUTF8 = '1'
[Console]::OutputEncoding = [Text.UTF8Encoding]::new($false)
uv run --project fashion-ai-runtime fashion-evaluate-candidate-readiness candidate-validation-manifest.json candidate-preflight-report.json visual-trial-report.json candidate-readiness-evidence.json candidate-readiness-report.json
```
