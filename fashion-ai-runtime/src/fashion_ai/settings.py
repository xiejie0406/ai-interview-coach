...
class RuntimeSettings:
    ...
    Provider 固定 disabled。认证密钥没有默认值；未配置时内部接口 fail closed。
    """
    ...
    def __post_init__(self) -> None:
        if self.provider_mode is not ProviderMode.DISABLED:
            raise ValueError("当前切片只允许 disabled Provider")
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "RuntimeSettings":
        ...
        try:
            ...
            raise ValueError("当前切片只允许 disabled Provider") from exc

        active_key = _read_key_pair(source, "ACTIVE")
        previous_key = _read_key_pair(source, "PREVIOUS")
...
def _read_key_pair(source: Mapping[str, str], slot: str) -> HmacKey | None:
    key_id = source.get(f"FASHION_AI_AUTH_{slot}_KEY_ID")
    encoded_secret = source.get(f"FASHION_AI_AUTH_{slot}_KEY_BASE64")
    if key_id is None and encoded_secret is None:
        return None
    if not key_id or not encoded_secret:
        raise ValueError(f"{slot.lower()} key id 与 Base64 key 必须同时配置")
    return HmacKey(
        key_id=key_id,
        secret=_decode_base64_key(encoded_secret, slot),
    )
