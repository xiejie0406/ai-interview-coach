...
from fashion_ai.settings import SERVICE_NAME, SERVICE_VERSION, RuntimeSettings
...
def create_app(
    *,
    ...
    """创建运行时应用；默认依赖始终为 disabled adapter。"""

    runtime_settings = settings or RuntimeSettings.from_env()
    if runtime_settings.provider_mode.value != "disabled":
    ...
    app = FastAPI(
        ...
        description="AI 智能选品内部 Python 运行时；当前 Provider 未启用。",
        docs_url=None,
        redoc_url=None,
        openapi_url=None,
    )
    ...
    app.include_router(
        create_router(
            analyzer or DisabledRequirementAnalyzer(),
            product_suggester or DisabledProductAttributeSuggester(),
            SelectionStylingWorkflow(selection_ranker or DisabledSelectionRanker()),
        )
    )
    app.add_middleware(
